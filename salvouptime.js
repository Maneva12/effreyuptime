...
> .ekle (Glitch Show Linki)
  `
          ...
          .setImage(
            "https://cdn.discordapp.com/attachments/928338928048689162/1009094219681505410/standard.gif"
          )
          .setThumbnail(message.author.avatarURL);
...
☀️ **  Bu Linklerden Sadece \`${
      db.get(`Sahiplik_${message.author.id}`) || null
...
client.on("message", async (message) => {
  ...
  if (!["477050335685509124"].includes(message.author.id)) return;
...
client.on("message", (message) => {
  ...
  if (
    ...
    const keslan = new Discord.MessageEmbed()
      .setDescription(
        `- **Projeniz 3 dakika içinde eklenicektir :) ${message.author}**
